...
from dataset import DatasetMesh, DatasetNERF, DatasetLLFF, DatasetTensoIR
...
def validate_itr(glctx, target, result_mesh, lgt, FLAGS, denoiser, iter=0):
    result_dict = {}
    with torch.no_grad():
        optix_ctx = ou.OptiXContext()

        buffers = render.render_mesh(FLAGS, glctx, result_mesh, target['mvp'], target['campos'], target['light'] if lgt is None else lgt, target['resolution'],
                                        spp=target['spp'], num_layers=FLAGS.layers, background=target['background'], optix_ctx=optix_ctx, 
                                        denoiser=denoiser, bsdf='pbr')
        ...
        if FLAGS.display is not None:
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    img = render.render_mesh(FLAGS, glctx, result_mesh, target['mvp'], target['campos'], target['light'] if lgt is None else lgt, target['resolution'],
                                                spp=target['spp'], num_layers=FLAGS.layers, background=white_bg, bsdf=layer['bsdf'], optix_ctx=optix_ctx)['shaded']
                    if layer['bsdf'] == 'kd':
...
def validate(glctx, result_mesh, lgt, dataset_validate, out_dir, FLAGS, denoiser):
    ...
    with open(os.path.join(out_dir, 'metrics.txt'), 'w') as fout:
        ...
        for it, target in enumerate(dataloader_validate):
            ...
            result_image, result_dict = validate_itr(glctx, target, result_mesh, lgt, FLAGS, denoiser)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='tensoir', type=str, choices=['llff', 'nerf', 'tensoir'])
    ...
    validate(glctx_display, result_mesh, lgt, dataset_validate, os.path.join(FLAGS.out_dir, "validate"), FLAGS, denoiser)
